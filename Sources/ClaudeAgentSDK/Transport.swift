...
public actor SubprocessCLITransport: TransportProtocol {
    ...
    private func buildArguments() throws -> [String] {
        ...
        if let outputFormat = options.outputFormat {
            let jsonData = try JSONSerialization.data(withJSONObject: outputFormat.mapValues { $0.value })
            let jsonString = String(data: jsonData, encoding: .utf8) ?? "{}"
    ...
    public func connect() async throws {
        ...
        if let cwd = options.cwd {
            guard FileManager.default.fileExists(atPath: cwd) else {
                throw ClaudeSDKError.connectionFailed(reason: "Working directory does not exist: \(cwd)")
            }
        ...
        do {
            ...
            throw ClaudeSDKError.connectionFailed(reason: "Failed to start CLI: \(error.localizedDescription)")
        }
        ...
        if let callback = options.stderrCallback {
            stderrTask = Task {
                await readStderr(callback: callback)
            }
        }
    ...
    private func readStderr(callback: @Sendable @escaping (String) -> Void) async {
    ...
    public func readMessages() -> AsyncThrowingStream<[String: Any], Error> {
        AsyncThrowingStream { continuation in
            Task {
                do {
                    try await self.readMessagesLoop(continuation: continuation)
                } catch {
    ...
    private func readMessagesLoop(continuation: AsyncThrowingStream<[String: Any], Error>.Continuation) async throws {
        ...
        while !Task.isCancelled {
            ...
            if buffer.count > maxBufferSize {
                throw ClaudeSDKError.bufferOverflow(bufferSize: buffer.count, maxSize: maxBufferSize)
            }
            ...
            while let newlineIndex = buffer.firstIndex(of: "\n") {
                ...
                do {
                    ...
                    // Speculative parsing - might be incomplete JSON, continue accumulating
                    // Only throw if we're confident it's malformed
                    if trimmed.hasPrefix("{") && trimmed.hasSuffix("}") {
                        throw ClaudeSDKError.jsonDecodeError(line: trimmed, underlyingError: error)
                    }
                }
            }
        }
        ...
        if let proc = process {
            ...
            if proc.terminationStatus != 0 {
                ...
                throw ClaudeSDKError.processError(exitCode: proc.terminationStatus, stderr: stderrOutput)
            }
        }
