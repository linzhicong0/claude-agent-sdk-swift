...
public actor ControlProtocol {
    ...
    public func initialize() async throws -> [String: Any] {
        ...
        return try await sendControlRequest(request, timeout: defaultControlTimeout)
    }
    
    /// Send an interrupt request to cancel ongoing operation
    public func interrupt() async throws {
        let request: [String: Any] = ["subtype": "interrupt"]
        _ = try await sendControlRequest(request, timeout: 5.0)
    }
    ...
    public func setModel(_ model: String) async throws {
        ...
        _ = try await sendControlRequest(request, timeout: 10.0)
    }
    ...
    public func rewindFiles(to checkpointId: String) async throws {
        ...
        _ = try await sendControlRequest(request, timeout: 30.0)
    }
    
    // MARK: - Control Request Sending
    
    /// Send a control request and wait for response
    private func sendControlRequest(_ request: [String: Any], timeout: TimeInterval) async throws -> [String: Any] {
        ...
        return try await withThrowingTaskGroup(of: [String: Any].self) { group in
            ...
            // Send the request after registration
            ...
            await removePendingRequest(requestId: requestId)
    ...
    private func registerPendingRequest(requestId: String, continuation: CheckedContinuation<[String: Any], Error>) {
    ...
    public func handleControlResponse(_ data: [String: Any]) {
        ...
        if let error = response["error"] as? String {
            pending.continuation.resume(throwing: ClaudeSDKError.controlProtocolError(reason: error))
        } else {
    ...
    private func handleCanUseTool(_ request: [String: Any]) async throws -> [String: Any] {
        ...
        guard let toolName = request["tool_name"] as? String,
              let toolInput = request["tool_input"] as? [String: Any] else {
    ...
    private func handleHookCallback(_ request: [String: Any]) async throws -> [String: Any] {
        guard let eventStr = request["hook_event"] as? String,
              let event = HookEvent(rawValue: eventStr) else {
            ...
            for hook in matcher.hooks {
                ...
                do {
                    output = try await withThrowingTaskGroup(of: HookOutput.self) { group in
                        ...
                        group.addTask {
                            try await Task.sleep(nanoseconds: UInt64(timeout * 1_000_000_000))
                            throw ClaudeSDKError.timeout(operation: "hook_callback", duration: timeout)
                        }
                ...
                if let specific = output.hookSpecificOutput {
                    response["hookSpecificOutput"] = [
                        ...
                        "permissionDecision": specific.permissionDecision as Any,
                        "permissionDecisionReason": specific.permissionDecisionReason as Any
                    ]
                }
                
                // If hook blocked, return immediately
                if output.decision == .block || !output.shouldContinue {
                    return response
                }
            }
        }
    ...
    private func handleMCPMessage(_ request: [String: Any]) async throws -> [String: Any] {
        guard let serverName = request["server_name"] as? String,
              let message = request["message"] as? [String: Any] else {
