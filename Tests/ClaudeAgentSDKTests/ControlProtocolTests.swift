...
final class ControlProtocolTests: XCTestCase {
    ...
    // MARK: - Hook Decision Tests

    func testHookDecisionAllow() {
        let decision = HookDecision.allow
        XCTAssertEqual(decision.rawValue, "allow")
    }

    func testHookDecisionDeny() {
        let decision = HookDecision.deny
        XCTAssertEqual(decision.rawValue, "deny")
    }

    func testHookDecisionAsk() {
        let decision = HookDecision.ask
        XCTAssertEqual(decision.rawValue, "ask")
    }

    // MARK: - Hook Specific Output Tests

    func testPreToolUseHookSpecificOutput() {
        ...
        XCTAssertEqual(output.permissionDecisionReason, "Safe command")
        XCTAssertEqual(output.updatedInput?["command"] as? String, "echo hello")
    }

    func testPostToolUseHookSpecificOutput() {
        ...
        XCTAssertEqual(output.additionalContext, "Tool execution completed successfully")
    }

    func testUserPromptSubmitHookSpecificOutput() {
        let output = UserPromptSubmitHookSpecificOutput(
            additionalContext: "User submitted a code review request"
        )

        XCTAssertEqual(output.hookEventName, .userPromptSubmit)
        XCTAssertEqual(output.additionalContext, "User submitted a code review request")
    }

    func testHookSpecificOutputWithAllFields() {
        ...
        XCTAssertEqual(output.permissionDecisionReason, "Blocked for safety")
        XCTAssertEqual(output.additionalContext, "Some context")
        XCTAssertEqual(output.updatedInput?["modified"] as? Bool, true)
    }
}
